...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('rifledeauth')
    .setDescription('Remove Rifle Certification from a user.')
  ...
  async execute(interaction, client) {
    ...
    try {
      ...
      if (!currentValue || currentValue === 'FALSE') {
        await interaction.reply({ content: '⚠️ This user is already deauthorized for Rifle.', ephemeral: true });
        return;
      }
      ...
      await interaction.reply({ content: `✅ Rifle certification removed for ID \`${discordId}\`.`, ephemeral: true });
      ...
      await logCommand({
        ...
        user: interaction.user,
        status: 'Success',
        description: `✅ Set cell N${rowIndex + 12} to FALSE for ID \`${discordId}\`.`,
      });
    } catch (err) {
      console.error(err);
