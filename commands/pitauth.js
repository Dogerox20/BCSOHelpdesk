...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('pitauth')
    .setDescription('Authorize PIT Certification for a user.')
  ...
  async execute(interaction, client) {
    ...
    try {
      ...
      if (currentValue === 'TRUE') {
        await interaction.reply({ content: '⚠️ This user is already authorized for PIT.', ephemeral: true });
        return;
      }
      ...
      await interaction.reply({ content: `✅ PIT certification authorized for ID \`${discordId}\`.`, ephemeral: true });
      ...
      await logCommand({
        ...
        user: interaction.user,
        status: 'Success',
        description: `✅ Set cell O${rowIndex + 12} to TRUE for ID \`${discordId}\`.`,
      });
    } catch (err) {
      console.error(err);
